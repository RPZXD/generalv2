...
<div class="wrapper">
  ...
  <div class="content-wrapper">
        ...
        <section class="content">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
                        ...
                        <button id="create-club-btn" class="inline-block bg-blue-600 hover:bg-blue-700 text-white font-semibold px-4 py-2 rounded shadow flex items-center gap-2" >
                            <span class="text-base">➕</span>
                            <span>สร้างชุมนุม</span>
                        </button>
                    </div>
                </div>
            </div>
            ...
            <!-- Modal สร้างชุมนุม -->
            <div id="create-club-modal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 hidden">
                <div class="bg-white rounded-lg shadow-lg w-full max-w-md">
                    <div class="flex justify-between items-center border-b px-4 py-2">
                        <h3 class="text-lg font-semibold">สร้างชุมนุมใหม่</h3>
                        <button id="close-modal-btn" class="text-gray-500 hover:text-gray-700 text-2xl">&times;</button>
                    </div>
                    <div class="p-4">
                        <!-- ตัวอย่างฟอร์ม (สามารถปรับแต่งได้) -->
                        <form id="create-club-form">
                            <div class="mb-3">
                                <label class="block font-medium mb-1">ชื่อชุมนุม</label>
                                <input type="text" class="border rounded w-full px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-400" name="club_name" required>
                            </div>
                            <div class="mb-3">
                                <label class="block font-medium mb-1">รายละเอียด</label>
                                <textarea class="border rounded w-full px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-400" rows="4" name="description" required></textarea>
                            </div>
                            <div class="mb-3">
                                <label class="block font-medium mb-1">ระดับชั้นที่เปิด</label>
                                <div id="create-grade-levels-checkboxes" class="flex flex-wrap gap-3">
...
$(document).ready(function() {
    ...
    $('#create-club-btn').on('click', function() {
        $('#create-club-modal').removeClass('hidden');
    });
    $('#close-modal-btn, #cancel-modal-btn').on('click', function() {
        $('#create-club-modal').addClass('hidden');
    });
    // (Optional) Prevent form submit for demo
    $('#create-club-form').on('submit', function(e) {
        e.preventDefault();
        var grades = [];
        $('#create-grade-levels-checkboxes input[name="grade_levels[]"]:checked').each(function() {
        ...
        $.ajax({
            url: '../controllers/ClubController.php?action=create',
            method: 'POST',
            data: $.param(formData),
            dataType: 'json',
            success: function(res) {
                if (res.success) {
                    $('#create-club-modal').addClass('hidden');
                    $('#create-club-form')[0].reset();
                    $('#club-table').DataTable().ajax.reload();
                    setTimeout(function() {
                        Swal.fire('สำเร็จ', 'สร้างชุมนุมใหม่เรียบร้อยแล้ว', 'success');
                    }, 300);
                } else {
                    Swal.fire('เกิดข้อผิดพลาด', res.message || 'เกิดข้อผิดพลาดในการบันทึกข้อมูล', 'error');
                }
            },
            error: function() {
    ...
    $('#club-table').on('click', '.delete-btn', function() {
        var clubId = $(this).data('id');
        Swal.fire({
            title: 'คุณต้องการลบชุมนุมนี้ใช่หรือไม่?',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '../controllers/ClubController.php?action=delete',
                    method: 'POST',
                    data: { club_id: clubId },
                    dataType: 'json',
                    success: function(res) {
                        if (res.success) {
                            ...
                            Swal.fire('เกิดข้อผิดพลาด', res.message || 'เกิดข้อผิดพลาดในการลบข้อมูล', 'error');
                        }
                    },
                    error: function() {
                        Swal.fire('เกิดข้อผิดพลาด', 'เกิดข้อผิดพลาดในการเชื่อมต่อเซิร์ฟเวอร์', 'error');
